    ...
    while True:
        ...
        try:
            ...
            if D.startswith('[') and D.endswith(']'):
                ...
                raise SyntaxError(f"Cannot parse the following string as JSON:\n\n{D}")
...
def parse_page(document, page):
    ...
    if header not in document:
        return ""
    return header + document.split(header)[1].split(footer)[0] + footer


def split_pages(document):
    chunks = document.strip().split('\n\n<PAGE')
    return [chunks[0]] + [f"<PAGE{chunk}" for chunk in chunks[1:]]
...
def chunk_document(document, chunk_size, token_counter):
    ...
    return [''.join(pages[i:j]).strip() for i, j in zip(idxs[:-1], idxs[1:])]
